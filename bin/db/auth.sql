...
create schema if not exists favz_public;
...
create or replace function
favz_public.request_password_reset(email text) returns void
  language plpgsql
  as $$
declare
  tok uuid;
...
create or replace function
favz_public.reset_password(email text, token uuid, pass text)
  returns void
  language plpgsql
  as $$
declare
...
create or replace view favz_public.users as
...
create or replace function
favz_public.update_users() returns trigger
language plpgsql
AS $$
begin
...
drop trigger if exists update_users on favz_public.users;
create trigger update_users
  instead of insert or update or delete on
    favz_public.users for each row execute procedure favz_public.update_users();


    create or replace function favz_public.signup(email text, pass text) returns void
as $$
  insert into basic_auth.users (email, pass, role) values
    (signup.email, signup.pass, 'user_auth');
$$ language sql;

drop type if exists favz_public.jwt_claims cascade;
create type favz_public.jwt_claims AS (role text, email text, exp integer);

create or replace function
favz_public.login(email text, pass text) returns favz_public.jwt_claims
  language plpgsql
  as $$
declare
  _role name;
...
ALTER DATABASE favz SET jwt.claims.email TO '';

create function favz_public.current_user() returns favz_public.users as $$
  select *
  from favz_public.users
  where email = current_setting('jwt.claims.email')
$$ language sql stable;

create role anon;
CREATE ROLE authenticator NOINHERIT LOGIN SUPERUSER;
alter role authenticator with password 'As121212';
create role user_auth;
grant user_auth to authenticator;
grant anon to authenticator;

grant usage on schema favz_public, basic_auth to anon, user_auth;

-- anon can create new logins
grant insert on table basic_auth.users, basic_auth.tokens to anon, user_auth;
grant select on table pg_authid, basic_auth.users, favz_public.users to anon, user_auth;
grant execute on function
  favz_public.login(text,text),
  favz_public.request_password_reset(text),
  favz_public.reset_password(text,uuid,text),
  favz_public.signup(text, text),
  favz_public.current_user()
  to anon, user_auth;
